...
class HybridCryptoSystem:
    ...
    def encrypt_with_custom_keys(self, input_file: str, output_file: str, public_key_path: str,
                                 key_size: int = 256) -> None:
    ...
    def decrypt_with_custom_keys(self, input_file: str, output_file: str, private_key_path: str) -> None:
...
def main():
    ...
    parser.add_argument('-s', '--settings', required=True, help='Путь к файлу настроек settings.json')

    subparsers = parser.add_subparsers(dest='command', required=True)

    gen_parser = subparsers.add_parser('gen', help='Генерация ключей')
    gen_parser.add_argument('--key-size', type=int, default=256, choices=[128, 192, 256],
    ...
    custom_enc_parser.add_argument('--key-size', type=int, default=256, choices=[128, 192, 256],
                                   help='Размер симметричного ключа')
    ...
    match args.command:
        ...
        case 'custom-enc':
            crypto_system.encrypt_with_custom_keys(args.input, args.output, args.pub_key, args.key_size)
        case 'custom-dec':
            crypto_system.decrypt_with_custom_keys(args.input, args.output, args.priv_key)
        case _:
