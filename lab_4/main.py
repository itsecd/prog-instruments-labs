...
class MainWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        logger.debug("Главное окно инициализируется")
        ...
        try:
            with open("userFiles/data.json", "r", encoding="utf8") as file:
                logger.info("Открытие файла с заметками")
                self.file = json.load(file)
                self.notes_dict = self.file["notes_dict"]
                self.last = self.file["last"]
                self.redraw_list_menu()
                self.select_item(self.last)
        except FileNotFoundError:
            logger.warning("Файл userFiles/data.json не найден, создаем новый")
        ...
        self.actionSaveNote.triggered.connect(lambda: self.about_quit.save_changes())
        self.actionRemoveNote.triggered.connect(lambda: self.delete_note())
        self.actionAbout.triggered.connect(lambda: self.about_window.help_show())
    ...
    def select_item(self, item_name: Optional[str]) -> None:
        ...
        """
        logger.debug(f"Выбор заметки: {item_name}")
    ...
    def redraw_list_menu(self) -> None:
        ...
        """
        logger.debug("Перерисовка списка заметок")
        self.notesList.clear()
        for element in self.notes_dict.keys():
            self.last = QtWidgets.QTreeWidgetItem(
                self.notesList, 
                [self.notes_dict[element]["title"], self.notes_dict[element]["date"]]
            ).text(0)
    ...
    def note_unsaved(self, status: bool = True) -> None:
        ...
        """
        logger.debug(f"Статус несохраненной заметки: {status}")
        if status:
    ...
    def load_note(self, item: Optional[QTreeWidgetItem], last_item: Optional[QTreeWidgetItem]) -> None:
        ...
        """
        logger.debug(f"Загрузка заметки: {item.text(0) if item else 'None'}")
        if item != last_item and item is not None:
    ...
    def delete_note(self) -> None:
        ...
        self.last = None
        if self.notesList.selectedItems():
    ...
    def get_note_text(self) -> str:
        ...
        Returns:
            str: Текст заметки.
        """
        return self.noteTextEdit.toPlainText()
...
class TableWidgetItem(QtWidgets.QTableWidgetItem):
    ...
    def __lt__(self, other: 'TableWidgetItem') -> bool:
        ...
        """
        logger.debug("Сравнение элементов таблицы")
...
class AboutWindow(QWidget):
    ...
    def help_show(self) -> None:
        ...
        """
        logger.debug("Показать/Скрыть окно 'О программе'")
        if self.isVisible():
            self.close()
        else:
...
class AboutQuit(QWidget):
    ...
    def save_changes_window(self) -> None:
        ...
        """
        logger.info("Показать окно 'Сохранить изменения'")
        self.move(
    ...
    def save_changes(self) -> None:
        ...
        """
        logger.info("Сохранить изменения в заметке")
        time_now = datetime.datetime.now()
        current_date_seconds_from_start = time_now.timestamp()
        current_date = str(time_now.strftime("%H:%M %d.%m.%Y"))

        if mainWin.last is None:
            if mainWin.get_note_title() not in mainWin.notes_dict:
                note_key = "untitled" if not mainWin.get_note_title().strip() else mainWin.get_note_title()
                mainWin.notes_dict[note_key] = {
    ...
    def delete_changes(self) -> None:
        ...
        Удаление изменений
        """
        logger.info("Удаление изменений")
        mainWin.note_unsaved(False)
        with open("userFiles/data.json", "w", encoding="utf8") as file:
            json.dump(mainWin.file, file, indent=4)
        app.closeAllWindows()
...
def except_hook(cls: type, exception: Exception, traceback: Optional[traceback]) -> None:
    ...
    """
    logger.critical("Необработанное исключение", exc_info=(cls, exception, traceback))
