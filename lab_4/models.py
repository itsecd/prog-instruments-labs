...
class PoetryModel(nn.Module):
    ...
    def generate(self, x, stoi, poetry_type=PoetryType.BEGIN, sent_num=4, max_len=15):
        ...
        for i in range(sent_num):
            if i == 0 and poetry_type == PoetryType.BEGIN:
                seq_out, hn = self.gru(x, hn)
                seq_out = seq_out[:, -1, :].unsqueeze(1)
                output.append(x)
            if poetry_type == PoetryType.HIDDEN_HEAD:
