...
while k <= 100000:                                                      #Конечное количество точек
    for i in range(1,len(files)):
        ...
        #поиск контрольных точек и дескрипторов - SIFT
        sift = cv2.xfeatures2d.SIFT_create(k)
        ...
        sko += sko
    sko = sko/50
    print "[SIFT] average standard deviation of the sequence from ", k, "points:", sko
    sko = 0
    k *= 2                                    #Шаг увеличения количества точек
#вывод примера на экран
img3 = cv2.drawMatches(mov, kp1, ref, kp2, matches[:20], None)
cv2.imshow("Keypoint matches SIFT", img3)
cv2.imshow("Registered image SIFT", out)
cv2.waitKey(0)