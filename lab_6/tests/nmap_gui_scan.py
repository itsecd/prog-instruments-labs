...
def build_nmap_command(target, port_option, custom_ports, syn_scan, enable_A, timing, extra_args, output_basename):
    ...
    # Advanced - должна идти ПОСЛЕ типа сканирования
    if enable_A:
        cmd.append("-A")
    ...
    if port_option == "all":
        ...
        p = custom_ports.strip() if custom_ports else ""
        if p:
            cmd.extend(["-p", p])
    ...
    if extra_args:
        # split safely
        try:
            extra_parts = shlex.split(str(extra_args))
            cmd.extend(extra_parts)
        except Exception:
            # Если не удается разобрать, просто пропускаем
            pass

    # Output base name (nmap will append .nmap .xml .gnmap with -oA)
    if output_basename:
        cmd.extend(["-oA", str(output_basename)])
