...
class Medicine(object):
    ...
    def possession_ratio_lower_bound_date(self, day_count):
        time_delta = timedelta(days = day_count)
        return self.possession_effective_end_date() - time_delta
        
    def possession_effective_start_date(self, day_count):
        return max( self.possession_ratio_lower_bound_date(day_count) , 
                   self.initial_dispense_date() )

    def prescriptions_in_range(self, day_count):
        effective_range = self._possession_effective_range(day_count)
        return filter( lambda p: 
            p.dispense_date in effective_range
              or p.completion_date() in effective_range, self.prescriptions )
    ...
    def _possession_effective_range(self, day_count):
        ...
        return [ start_date + timedelta(days = i) for i in 
    ...
    def dates_prescribed(self, day_count):
        ...
        for p in self.prescriptions_in_range(day_count):
            dispense_days = {
                p.dispense_date + timedelta(days = i)
                for i in range(p.days_supply) }
    ...
    def dates_prescribed_in_effective_range(self, day_count):
        ...
        return filter( lambda d: 
                      d in possession_effective_range , 
                      self.dates_prescribed(day_count) )
