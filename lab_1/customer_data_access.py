...
class CustomerDataAccess:
    ...
    def loadCompanyCustomer(self, externalId: str, companyNumber: Optional[str]) -> CustomerMatches:
        ...
        if matchByExternalId is not None:
            ...
            matchByMasterId: Customer = self.customerDataLayer.findByMasterExternalId(
                externalId
            )
            if matchByMasterId is not None:
                matches.add_duplicate(matchByMasterId)
        else:
            matchByCompanyNumber: Customer = self.customerDataLayer.findByCompanyNumber(
                companyNumber
            )
    ...
    def createCustomerRecord(self, customer: Customer) -> Customer:
        ...
        self.cursor.execute(
            "INSERT INTO customers VALUES (?, ?, ?, ?, ?, ?, ?);", (
                customer.internalId, customer.externalId,
                customer.masterExternalId, customer.name,
