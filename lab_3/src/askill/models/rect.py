...
class Rect(Action):
    ...
    def __repr__(self):
        return f"Rect({self.x1}, {self.y1}, {self.x2}, \
            {self.y2}, {repr(self.border)}, {repr(self.fill)})"

    def construct(self, canvas: Canvas):
        for row in range(self.y1, self.y2 + 1):
            for col in range(self.x1, self.x2 + 1):
                if row < 0 or col < 0 or row >= canvas.height \
                        or col >= canvas.height:
                    continue

                if row == self.y1 or row == self.y2 or col == self.x1 \
                        or col == self.x2:
